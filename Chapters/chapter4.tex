...
The complexity penalization term can prevent the GP from oversmoothing the data.
...
we can reconstruct the signal using a Wiener filter (Wiener 1949). 
...
\begin{equation}
	p(\lensparams| \gpparams) = \mathcal{N}(\mu_\lensparams,
...
\mathsf{N})^{-1}\mathsf{N}.
\end{align}
We use eq. \ref{eq:Wiener_filter_distribution} to process two sets of
...
\ref{eq:kernel_derivatives2} can be slow.
...
to probe the space of possible GP parameters. But the scalability of our method
is limited.
...
\citealt{Rasmussen2006}). The inversion of the matrix can be sped up to
$O(M^2 n_{\rm gal})$ for
...
as automatic differentiation can spreads factors of the derivatives over a tree structure for
almost exact computation. We note that the most urgent tasks for showing the
strength of the GP model is to 
