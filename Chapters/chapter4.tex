...
% early universe. 
We know how the initial matter perturbations represented by the CMB could grow
...
In Chapter 1, we have provided a review of the basics of the gravitational lensing physics. 
Here we discuss how to incorporate the physics as part of a statistical model. 
For this, we need to know the properties of our data and the signal.
...
comes from the lensing physics. 
Under the Born approximation, we have shown in Chapter 1 that  
the lensing observables are the second derivatives of the
a scalar lensing potential. We therefore expect the lensing observables to be curl-free.
...
\begin{figure}
	...
	\caption{
		...
		of length 20. The data points (($\psi, \xv_0$); black dots) 
...
($\gpparams$). Since the noise contributions are independent and identically distributed (i.i.d) as a Gaussian,
...
The resulting composite kernel matrix for the observed data is:
\begin{equation}
...
The corresponding conditional distribution is \citep{Rasmussen2006}:
\begin{align}
...
\begin{equation}
	\mathbb{E}(\psi | \psi_0) = \kerngp(\xv, \xv_0)[\kerngp'(\xv_0, \xv_0) +
...
statistical model $M$. 
In the case of the example in this section, the data vector is ${\bf d} = \psi_0$, and 
our model is the GP. We readily write down the form of the log likelihood \citep{Rasmussen2006} as:
\begin{align}
	\log \mathcal{L} &= \log (Pr(\psi | \kerngp', \xv))\\
	&=  -\frac{1}{2}\psi_0^T \kerngp^{-1}\psi_0
- \frac{1}{2}\log|\kerngp'| - \frac{n}{2}\log(2\pi),
	\label{eq:log_marginal_likelihood0}
\end{align}
where $n=20$ as we used 20 data points and this expression gives a scalar output.
...
all the eigenvalues) spanned by the kernel. 
...
\right) \kerngp_{mn},\\ 
\end{align*}
where $\kerngp_{mn}$ represents each element of the exponential squared kernel.
...
convergence $\kappa$, or simply compute it with the mean prediction expression
in eq. \ref{eq:conditional_distribution}. 
...
before.   
As the lensing signal from Abell 781 is high, we see an agreement of the 
inferred $\lensparams$ with \cite{Wittman2014}. 
