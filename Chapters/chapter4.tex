...
Please refer to Schneider et al. (in prep.) for more details.
...
are centered on the galaxy cluster, Abell 781, which has an estimated redshift
of $z \approx 0.43$ \citep{Wittman2014}. A galaxy cluster is a
...
It shows features that are general agreement between the true signal. 
...
become negligible. We will discuss ways to speed up the computation in section
\ref{subsec:computational_performance}. 

Our GP posterior map for the {\sc GalSim} data also retains very little B-mode
features. The B-mode amplitudes are $< 2e^{-3}$ in the center of the field,  
and only have larger amplitudes $< 2e^{-2}$ at the field boundaries.  
...
\end{figure}
We also see a general agreement of the features in the mass map of Abell 781 in
Fig. \ref{fig:Abell781_massmap} with previous studies of the same cluster \cite{Wittman2014}. 
We are restricted by the Nyquist frequency to give a coarser map as discussed
before. Due to the small number of galaxies used for probing $\lensparams$ and
the lack of characterization of the systematics, we
have not yet pursued the inference of cosmological parameters from the DLS data.
...
operations can then provide speedups from looping over less data entries.
The relevant field of literature that studies how the sparsity
...
For a best illustration for pattern discovery with a GP, see \cite{Duvenaud2013} which
shows an example of how kernels of different periodicity can represent 
the short and long terms trends for carbon dioxide emission at Mauna Loa.
...
simulated signal or with past analyses.  
...
from either the GP parameters or the realization of the lensing observables
$\Upsilon$ remains to be proved. 
