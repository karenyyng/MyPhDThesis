...
With the use of two graphical models, we will also demonstrate how our method
...
\end{equation}
where we use the symbol $(\lensparams', \xv')$ to denote the unobserved set of data,
and the $(\lensparams, \xv)$ for the existing (training) data.
...
%   of each galaxy is 
% independent and each follow a Gaussian distribution, we can model  
% the shape noise variance ($\sigma_e$) as part of a composite GP kernel. 
% Furthermore, we can also model the measurement error $\epsilon_{\rm int}$ of
% each galaxy to be drawn independently from a Gaussian with a zero mean.
% % (The possible use of composite GP kernels can be found
% in detail in \cite{Duvenaud2013} and \cite{Wilson2013}).
\begin{equation}
...
data for demonstration purposes. During the computation of the posterior
realizations, we used the optimized $a_{\rm GP}^*$ values as a prior. 
An representation of our map-making inference procedure can be seen in Fig. 
...
The first set of synthetic data that we analyzed were generated via the following procedure: 
We first generate a angular power spectrum of the convergence signal using {\sc
...
With the convergence power spectrum, we can then generate lensed   
galaxy ellipticities generated using {\sc GalSim}, a modular galaxy image
simulation toolkit \citep{Rowe2015}.
In the process of generating the data, we have specified 
...
$\sigma_e^2$ from {\sc GalSim}, we make it deliberately small $\sigma_e^2 =
...
The second set of real data is obtained from the DLS, an optical imaging survey 
...
Although the B-mode amplitudes from our GP inference are small, improvements can be made 
if we understand how to fit the B-mode amplitudes to achieve complete E/B mode
separation.
...
\begin{figure}[h!]
	...
	\caption{A comparison of mass maps for Abell 781 in the Deep Lens Survey (DLS) 
		...
		There are four white crosses on each figure showing the location of inferred mass
		peak locations from previous studies. 
...
have not yet pursued the inference of cosmological parameters from the
...
There are efforts to explore different ways to relate the posterior lensing signal 
...
covariance kernel models) to cosmological parameters. 
We note that the GP parameters may not be very sensitive to cosmological parameters
due to its flexibility to model a large range of data. The sampling strategy of
the galaxy locations may also affect the sensitivity. A sensitivity test can be
done by repeating our {\sc GalSim} study using convergence power spectrum of different cosmology 
parameters and different sampling. It is also possible to use dimensionality
techniques to find the features in the posterior map that can best determine the
...
Our method is also not as computationally competitive as the 2PCF. One of
...
In terms of speed optimization, rather than relying on 
parallel computation of the kernel methods, there are several 
...
computational representations. Thus, numerical libraries for sparse matrix 
operations (such as {\sc ScaLAPACK} or {\sc PLAPACK})
can then provide speedups from looping over less data entries.
...
stacking of multiple exposures. Previous studies of modeling the systematics
have made use of interpolation for such a task (\citealt{Rowe2010},
\citealt{Jee2013a}). However, we have demonstrated a GP can accomplish
interpolate in a much more generalizable way.
...
fit systematics and incompleteness can allow better E / B mode separation. 

In addition, we note that there is no strong reason to fix the amplitude of the diagonal 
shape noise kernel in eq. \ref{eq:composite_kernel}. The amplitude
$\sigma_e^2$ can be inferred with a prior that brackets a reasonable range of
shape noise value.  
...
simulated signal or with past analytical techniques.  
