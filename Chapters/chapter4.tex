...
A multivariate Gaussian has dimension specified by the number of features 
of the input data vector.
If each galaxy observation contains 2 spatial dimensions, then the resulting
multivariate Gaussian only has 2 dimensions, the covariance matrix $\Sigma$ has
a size $2 \times 2$. However, this is not true for a GP. 
Even if the input galaxy data has two spatial dimensions, the visible dimension 
of a GP kernel matrix is of dimension $\ngal \times \ngal$.  
The mean vector function has the same length as input observations, e.g. number
of source galaxies.
The covariance kernel function $\kerngp(\xv, \yv)$
gives a matrix as outputs, and we will refer to it as one of the following names
interchangeably depending on the context, such as the 
...
The drawn collections of values $\psi$ from a GP carry the correlation structures
specified by the kernel $\kerngp(\xv, \yv)$: 
\begin{equation}
...
For example, given a list of galaxy 2D spatial coordinates $\xv$ or $\yv$, 
We can compute the covariance matrix kernel function. 
Part of the matrix kernel function is a distance matrix that 
measures how different or similar of the observed lensing observable 
$\lensparams$ are at different spatial coordinates $\xv$.
A convenient distance matrix uses Euclidean distance in the form of $r^2$: 
\begin{equation}
	r^2 \equiv (\xv - \yv)^T \Matrix{D} (\xv - \yv), 
\end{equation}
where $\Matrix{D}$ is a diagonal matrix.

One of these choices is the exponential squared kernel: 
\begin{equation}
...
\begin{figure}
	...
	\caption{Left: An illustration of a GP with a one-dimensional input vector ${\bf x}$. 
