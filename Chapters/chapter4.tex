...
% \subsection{Comparison to other representations of lensing information}
...
% is insufficient to summarize all the information for the underlying Gaussian distribution.
% Estimators of the lensing information includes 2PCF, the power spectrum, 
% the mass aperture statistics and the shear peak statistics, to name a few.
% The summary statistics are the best-fit representation (realization) of the data based on some
% model assumptions. For the 2PCF, the implicit model assumption is that any
...
% It may be more reasonable to compare the summary statistic of the GP, such as
% the mean realization $\bar{\psi}$ to such estimators.  
% The uncertainty information of the aforementioned estimators, 
% needs to be evaluated separately with a covariance. When maximizing the
...
% 
% There is also an attempt to use the Karhunen-Lo\'{e}ve (KL) transform to decompose
% the lensing information into different eigenmodes \citep{VanderPlas2012a}.
% It is an attempt to use orthogonal basis to represent the 
% observed data.% , which is considered one instance of the model that generates the data. 
% While such approach may also offer great signal-to-noise separation, 
% an important advantage of the GP method over the KL eigenbases 
% is that a GP (which is a PDF of some form) 
% can be naturally incorporated as part of a probabilistic framework. 
% 
